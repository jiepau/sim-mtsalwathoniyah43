...
export function CetakDenahRuangDialog({ open, onOpenChange, sesi }: Props) {
  ...
  const ruangFiltered = useMemo(
    () => filterRuang === 'all' ? ruang : ruang.filter((r) => r.id === filterRuang), [ruang, filterRuang],
  );
  ...
  const seatMap = useMemo(() => {
    ...
    peserta.forEach((p) => {
      ...
      if (!inner) { inner = new Map(); out.set(p.ruang_id, inner); }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Denah Tempat Duduk — {sesi.nama}</DialogTitle></DialogHeader>

        <div className="no-print p-3 rounded-lg border bg-card">
          <Label className="text-xs">Filter Ruang</Label>
          <select className="h-9 w-64 border rounded-md px-2 text-sm ml-2"
        ...
        <PrintPreviewFrame preview={preview} orientation={orientation}>
          <div className="space-y-6">
              ...
              return (
                <div key={r.id} style={{ pageBreakAfter: idx < ruangFiltered.length - 1 ? 'always' : 'auto' }}>
                  <PrintKopMadrasah
                    ...
                    periode={tglText ? `Tanggal: ${tglText}` : undefined}
                  />
                        ...
                        <div key={seatNum}
                          ...
                          {p ? (
                            <>
                              ...
                              <div className="text-[7pt] leading-tight mt-0.5 line-clamp-2">
                                {p.siswa?.nama || '-'}
                              </div>
                            </>
                          ) : (
            ...
            {ruangFiltered.length === 0 && (
              <p className="text-center text-sm py-12">Belum ada ruang.</p>
            )}
          </div>
        </PrintPreviewFrame>
      </DialogContent>
