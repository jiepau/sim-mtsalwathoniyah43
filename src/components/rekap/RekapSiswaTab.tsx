...
import { Users, CalendarDays, CheckCircle, XCircle, AlertCircle, Clock } from 'lucide-react';
...
export function RekapSiswaTab({ selectedKelas, selectedTA, selectedBulan, selectedTahun, hariEfektif, bulanLabel, kelasName }: Props) {
  const [siswaList, setSiswaList] = useState<Siswa[]>([]);
  const [absensiRecords, setAbsensiRecords] = useState<{ siswa_id: string; status: string }[]>([]);
  ...
  useEffect(() => {
    if (!selectedKelas || !selectedTA || siswaList.length === 0) { setAbsensiRecords([]); return; }
  ...
  const rekapData = useMemo(() => {
    return siswaList.map(siswa => {
      ...
      return { id: siswa.id, nis: siswa.nis, nama: siswa.nama, jk: siswa.jenis_kelamin === 'Laki-laki' ? 'L' : 'P', hadir, sakit, izin, alfa, totalRecorded, belumDiisi: hariEfektif - totalRecorded, persentase };
    });
  }, [siswaList, absensiRecords, hariEfektif]);

  const grandTotal = useMemo(() => {
    const t = { hadir: 0, sakit: 0, izin: 0, alfa: 0 };
    rekapData.forEach(r => { t.hadir += r.hadir; t.sakit += r.sakit; t.izin += r.izin; t.alfa += r.alfa; });
    return t;
  }, [rekapData]);
  ...
  const exportData = useMemo(() => rekapData.map((r, idx) => ({
    no: idx + 1, nis: r.nis, nama: r.nama, jk: r.jk, hadir: r.hadir, sakit: r.sakit, izin: r.izin, alfa: r.alfa, hari_efektif: hariEfektif, persentase: `${r.persentase}%`,
  ...
  if (!selectedKelas || !selectedTA) {
    return (
      <Card><CardContent className="p-8 text-center text-muted-foreground">
        ...
        <p>Pilih tahun ajaran dan kelas untuk melihat rekap absensi siswa.</p>
      </CardContent></Card>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <Card className="border-border/50"><CardContent className="p-4 text-center"><Users className="h-5 w-5 mx-auto mb-1 text-muted-foreground" /><p className="text-2xl font-bold text-foreground">{rekapData.length}</p><p className="text-xs text-muted-foreground">Total Siswa</p></CardContent></Card>
        <Card className="border-border/50"><CardContent className="p-4 text-center"><CheckCircle className="h-5 w-5 mx-auto mb-1 text-emerald-600" /><p className="text-2xl font-bold text-foreground">{grandTotal.hadir}</p><p className="text-xs text-muted-foreground">Total Hadir</p></CardContent></Card>
        <Card className="border-border/50"><CardContent className="p-4 text-center"><AlertCircle className="h-5 w-5 mx-auto mb-1 text-amber-600" /><p className="text-2xl font-bold text-foreground">{grandTotal.sakit}</p><p className="text-xs text-muted-foreground">Total Sakit</p></CardContent></Card>
        <Card className="border-border/50"><CardContent className="p-4 text-center"><Clock className="h-5 w-5 mx-auto mb-1 text-blue-600" /><p className="text-2xl font-bold text-foreground">{grandTotal.izin}</p><p className="text-xs text-muted-foreground">Total Izin</p></CardContent></Card>
        <Card className="border-border/50"><CardContent className="p-4 text-center"><XCircle className="h-5 w-5 mx-auto mb-1 text-red-600" /><p className="text-2xl font-bold text-foreground">{grandTotal.alfa}</p><p className="text-xs text-muted-foreground">Total Alfa</p></CardContent></Card>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div>
              <CardTitle className="text-lg">Rekap Siswa — {bulanLabel} {selectedTahun} — {kelasName}</CardTitle>
            ...
            <ExportButton
              data={exportData}
              columns={[
                { header: 'No', accessor: d => d.no }, { header: 'NIS', accessor: d => d.nis }, { header: 'Nama Siswa', accessor: d => d.nama },
                { header: 'L/P', accessor: d => d.jk }, { header: 'Hadir', accessor: d => d.hadir }, { header: 'Sakit', accessor: d => d.sakit },
                { header: 'Izin', accessor: d => d.izin }, { header: 'Alfa', accessor: d => d.alfa }, { header: 'Hari Efektif', accessor: d => d.hari_efektif },
                { header: 'Persentase', accessor: d => d.persentase },
              ]}
              filename={`Rekap_Siswa_${kelasName}_${bulanLabel}_${selectedTahun}`}
            />
          </div>
        </CardHeader>
        <CardContent>
